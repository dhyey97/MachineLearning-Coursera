...
for i=1:m
  X_i = X(i,:);
	...
	y_i = zeros(1,K);
	y_i(y(i)) = 1;
	J = J + ( -1 * y_i * log(h_of_Xi)' - (1 - y_i) * log(1 - h_of_Xi)' );
endfor
J=J/m;
% Applying Regularization.
J=J+(lambda/(2*m))*(sum(sum(sumsq(Theta1(:,2:input_layer_size+1)))+sum(sumsq(Theta2(:,2:hidden_layer_size+1)))))
%
...
for i=1:m
  ...
  Theta1_grad=Theta1_grad+(delta2(:,2:hidden_layer_size+1)'*a1);
